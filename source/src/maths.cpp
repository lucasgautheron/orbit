...
double vec::magnitude() const { return sqrt(squaredlen()); }
vec &vec::normalize() { div(magnitude()); return *this; }

double vec::dist(const vec &e) const { vec t; return dist(e, t); }
double vec::dist(const vec &e, vec &t) const { t = *this; t -= e; return t.magnitude(); }

double vec::angle(const vec &e) const
{
    return acos(this->dot(e)/(this->magnitude()*e.magnitude()));
}
